...
export const Wizard: React.FC<WizardProps> = ({ onSuccess, onCancel }) => {
  ...
  const stepsList = [
    ...
    { num: 2, title: 'Assign Owner', desc: 'Assign container to user', icon: UserIcon },
    ...
    { num: 5, title: 'Storage size', desc: 'Define container root disk size', icon: HardDrive },
  ...
  return (
    <div className="w-full max-w-4xl al-card overflow-hidden flex flex-col h-[75vh]">
      <div className="p-4 border-b border-borderSubtle bg-white/5 flex items-center justify-between">
        <h2 className="text-sm font-semibold text-white">Create Container Wizard</h2>
        <span className="text-xs text-gray-500">{deploying ? 'Deploying...' : `Step ${step} of 8`}</span>
      </div>
        ...
        <div className="flex-1 p-6 overflow-y-auto space-y-6">
          ...
          {deploying && deployProgress && (
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-white">Deploying Container</h3>
              </div>
          ...
          {!deploying && step === 2 && (
            <div className="space-y-4 max-w-md">
              <h3 className="text-sm font-semibold text-white">Assign owner for this container</h3>
          ...
          {!deploying && step === 3 && (
            <div className="space-y-4">
              <h3 className="text-sm font-semibold text-white">Select OS Template / Container Image</h3>
              <div className="space-y-2">
                {templates.map(t => (
                  <button
                    key={t.path}
            ...
            <div className="space-y-4 max-w-md">
              ...
              <div className="space-y-4 text-xs">
                ...
                <div>
                  ...
                  <input type="range" min="512" max="16384" step="256"
                    className="w-full h-1 bg-white/10 rounded-lg appearance-none cursor-pointer"
                    value={memoryMb}
          ...
          {!deploying && step === 5 && (
            <div className="space-y-4 max-w-md">
              <h3 className="text-sm font-semibold text-white">Root Disk Allocation</h3>
              <div>
            ...
            <div className="space-y-4 max-w-md">
              ...
              <div className="space-y-3 text-xs">
                <div>
                  ...
                  <input type="text" className="w-full al-input"
                    value={netBridge}
                ...
                <div>
                  ...
                  <input type="text" placeholder="dhcp or 10.0.0.100/24" className="w-full al-input"
                    value={netIp}
            ...
            <div className="space-y-4 max-w-md">
              ...
              <div className="space-y-3 text-xs">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[11px] text-gray-400 block mb-1">Container Name</label>
                    <input type="text" placeholder="my-container" className="w-full al-input"
                      value={name}
                  ...
                  <div>
                    ...
                    <input type="number" className="w-full al-input"
                      value={vmid}
                ...
                <div>
                  ...
                  <input type="text" placeholder="web.cynexvm.local" className="w-full al-input"
                    value={hostname}
                ...
                <div>
                  ...
                  <input type="password" placeholder="••••••••" className="w-full al-input"
                    value={password}
      ...
      <div className="p-4 border-t border-borderSubtle bg-white/5 flex items-center justify-between">
        {deploying ? (
          <>
            <button disabled className="flex items-center gap-1 al-btn al-btn-secondary opacity-40 cursor-not-allowed">
              <ChevronLeft size={16} /> Back
            </button>
            <button disabled className="al-btn al-btn-primary px-6 opacity-60 cursor-not-allowed">
          ...
          <>
            <button onClick={() => { setError(null); setStep(8); }} className="flex items-center gap-1 al-btn al-btn-secondary">
              <ChevronLeft size={16} /> Back
            </button>
            <button onClick={handleDeploy} className="al-btn al-btn-primary px-6 flex items-center gap-1.5">
              <RefreshCw size={14} /> Retry Deploy
            </button>
          </>
        ) : (
          <>
            <button onClick={step === 1 ? onCancel : () => setStep(step - 1)} className="flex items-center gap-1 al-btn al-btn-secondary">
              <ChevronLeft size={16} /> {step === 1 ? 'Cancel' : 'Back'}
            </button>
            {step < 8 ? (
              <button onClick={() => setStep(step + 1)} className="flex items-center gap-1.5 al-btn al-btn-primary">
                Next <ChevronRight size={16} />
              </button>
            ) : (
              <button onClick={handleDeploy} className="al-btn al-btn-primary px-6 flex items-center gap-1.5" disabled={loading}>
                {loading ? (
                  <><Loader size={14} className="animate-spin" /> Queueing...</>
                ) : (
                  <><Terminal size={14} /> Deploy Container</>
                )}
              </button>
            )}
          </>

...
import { Server, Activity, Users, ShieldAlert, Cpu, HardDrive } from 'lucide-react';
...
export const AdminDashboard: React.FC = () => {
  ...
  const fetchStats = async () => {
    try {
      ...
      const instancesRes = await fetch('/api/v1/instances', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 m-8">
        ...
        <div className="bg-neutral-50 dark:bg-neutral-800/20 rounded-xl p-5 border border-neutral-200 dark:border-white/5">
          ...
          <p className="text-xs text-neutral-400 mt-2">Active customer credentials</p>
        </div>
      </div>
      ...
      <div className="mx-8 grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-white/5 rounded-xl p-6 border border-neutral-300 dark:border-neutral-800/20 shadow-sm space-y-4">
          <h3 className="text-sm font-semibold text-neutral-900 dark:text-white flex items-center gap-2">
            <Cpu className="text-blue-500" size={18} /> Cluster CPU Distribution
          </h3>
          <div className="space-y-2 text-xs">
            <div className="flex justify-between">
              <span className="text-neutral-400">Total Provisioned Cores</span>
              <span className="font-semibold text-neutral-900 dark:text-white">32 Cores</span>
            </div>
            <div className="flex justify-between">
              <span className="text-neutral-400">Allocated CPU Power</span>
              <span className="font-semibold text-neutral-900 dark:text-white">16 Cores (50% density)</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-white/5 rounded-xl p-6 border border-neutral-300 dark:border-neutral-800/20 shadow-sm space-y-4">
          <h3 className="text-sm font-semibold text-neutral-900 dark:text-white flex items-center gap-2">
            <HardDrive className="text-blue-500" size={18} /> Storage Pool Capacity
          </h3>
          <div className="space-y-2 text-xs">
            <div className="flex justify-between">
              <span className="text-neutral-400">Default local-lvm allocation</span>
              <span className="font-semibold text-neutral-900 dark:text-white">2.5 TB / 4.0 TB</span>
            </div>
            <div className="flex justify-between">
              <span className="text-neutral-400">Remaining space</span>
              <span className="font-semibold text-emerald-400">1.5 TB Available</span>
            </div>
          </div>
        </div>
      </div>
